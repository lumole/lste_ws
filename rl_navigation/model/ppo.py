...
def generate_action(env, state_list, policy, action_bound):
    if env.index == 0:
        ...
        s_list = torch.from_numpy(s_list).float().cuda()
        goal_list = torch.from_numpy(goal_list).float().cuda()
        speed_list = torch.from_numpy(speed_list).float().cuda()
...
def generate_action_no_sampling(env, state_list, policy, action_bound):
    if env.index == 0:
        ...
        # Robust conversion: ensure numeric numpy arrays (avoid object dtype)
        def _to_np_goal(x):
            if x is None:
        ...
        def _to_np_obs(x):
            if x is None:
                # default: 3 frames of OBS_SIZE zeros
                return np.zeros((LASER_HIST, OBS_SIZE), dtype=np.float32)
            try:
                # common case: x is deque/list/tuple of frame arrays
                frames = [np.asarray(f, dtype=np.float32) for f in x]
                return np.stack(frames, axis=0)
            except Exception:
                a = np.asarray(x, dtype=np.float32)
                # if 1D and matches OBS_SIZE, replicate to LASER_HIST frames
                if a.ndim == 1 and a.shape[0] == OBS_SIZE:
                    return np.stack([a] * LASER_HIST, axis=0)
                return a.astype(np.float32)

        # convert elementwise then stack
        s_conv = [_to_np_obs(x) for x in s_list]
        try:
            s_np = np.stack(s_conv, axis=0)
        except Exception:
            s_np = np.array(s_conv, dtype=np.float32)

        goal_np = np.stack([_to_np_goal(x) for x in goal_list], axis=0)
        speed_np = np.stack([_to_np_speed(x) for x in speed_list], axis=0)

        s_list = torch.from_numpy(s_np).float().cuda()
        goal_list = torch.from_numpy(goal_np).float().cuda()
        speed_list = torch.from_numpy(speed_np).float().cuda()
...
def ppo_update_stage1(policy, optimizer, batch_size, memory, epoch,
    ...
    for update in range(epoch):
        ...
        for i, index in enumerate(sampler):
            sampled_obs = torch.from_numpy(obss[index]).float().cuda()
            sampled_goals = torch.from_numpy(goals[index]).float().cuda()
            sampled_speeds = torch.from_numpy(speeds[index]).float().cuda()

            sampled_actions = torch.from_numpy(actions[index]).float().cuda()
            sampled_logprobs = torch.from_numpy(logprobs[index]).float().cuda()
            sampled_targets = torch.from_numpy(targets[index]).float().cuda()
            sampled_advs = torch.from_numpy(advs[index]).float().cuda()
